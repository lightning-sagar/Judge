...
// List of worker endpoints (for distribution logic)
  ...
  try {
    ...
    await Promise.all(
      Object.entries(workerTaskMap).map(async ([workerId, tcList]) => {
        ...
        // Push job to queue — push JSON with ques_name, workerId, and exact redisKey
        await redis.lPush(
          "job_queue",
          JSON.stringify({ ques_name, workerId, redisKey })
        );
      })
    );

    console.log(
      `[INFO] Job ${ques_name} queued for ${Object.keys(workerTaskMap).length} workers`
    );
    ...
    if (!completed) {
      throw new Error(
        `Timeout waiting for workers to finish for job ${ques_name}`
      );
    }
