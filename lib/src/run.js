...
export async function judge({
  ...
  try {
    ...
    for (let i = 0; i < batches.length; i++) {
      const batchName = `${ques_name}_batch_${i}`;
      await redis.rPush(`testcase_queue:${batchName}`, JSON.stringify(batches[i]));
      await redis.lPush("job_queue", batchName);
    }
    ...
    const waitUntilCompleted = async () => {
      ...
      const MAX_ATTEMPTS = 360; // 30 seconds
      let attempts = 0;
    ...
    const batchResults = await redis.lRange(`results_queue:${ques_name}`, 0, -1);
    let results = [];
    if (batchResults && batchResults.length > 0) {
      try {
        results = batchResults.map((r) => JSON.parse(r)).flat();
      } catch (err) {
        console.error("Error parsing batch results:", err);
      }
    } else {
      console.warn(`[WARN] No batch results found for job ${ques_name}`);
}
    return {
      status: "completed",
      jobId: ques_name,
